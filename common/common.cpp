...
common_init_result::common_init_result(common_params & params, bool model_only) :
    ...
    // warn when tiering is on but the model has no expert tensors
    if (params.cmoe && llama_expert_tier::expert_weight_bytes(*model) == 0) {
        LOG_WRN("%s: expert tiering is on but the model has no expert tensors; tiering defaults have no effect\n", __func__);
    }
    // warn when tiering is on with no manual -c: fit spends free VRAM on
    // context instead of the expert cache
    if (params.cmoe && params.n_ctx == 0 && llama_expert_tier::expert_weight_bytes(*model) > 0) {
        LOG_WRN("%s: -c not set: auto-fit will spend free VRAM on context instead of the expert cache; set -c and/or use -ctk q8_0 -ctv q8_0 to free VRAM for caching experts\n", __func__);
    }
...
struct llama_context_params common_context_params_to_llama(const common_params & params) {
    ...
    // Sequential tree-path fallback reuses the main sequence only, so n_seq_max
    // stays equal to n_parallel (keeps combined DFlash graph available).
