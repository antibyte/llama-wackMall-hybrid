...
struct common_speculative_impl {
    ...
    size_t n_call_process = 0; // number of times process() ran (feature inject / combined take).
    ...
    int64_t t_process_us = 0; // total time spent in process() (inject / combined take) in microseconds.
    int64_t t_accept_us  = 0; // total time spent in accumulation of this implementation in microseconds.

    // Optional fine-grained counters (draft-dflash populates these).
    int64_t t_draft_decode_us = 0;
    int64_t t_draft_sample_us = 0;
    int64_t t_ddtree_us       = 0; // top-K extract + tree build
    size_t  n_draft_blocks    = 0;
    size_t  n_draft_block_tok = 0;
    size_t  n_draft_out_tok   = 0;
    size_t  n_ddtree_builds   = 0;
    size_t  n_ddtree_nodes    = 0; // sum of tree.n_nodes over builds
    size_t  n_process_combined   = 0;
    size_t  n_process_standalone = 0;
...
struct common_speculative_impl_draft_dflash : public common_speculative_impl {
    ...
    // scratch buffer for concatenated target features [n_tokens, n_embd_enc]
    std::vector<float> features_buf;
    // scratch for full-vocab logits copy [L * n_vocab]
        ...
        , is_dspark(type == COMMON_SPECULATIVE_TYPE_DRAFT_DSPARK)
    {
        ...
        // DDTree env config (matches Lucebox flags where possible)
        {
            const char * en = std::getenv("LLAMA_DFLASH_DDTREE");
            ddtree_mode = en && en[0] != '\0' && en[0] != '0';
            if (const char * v = std::getenv("LLAMA_DFLASH_DDTREE_K")) {
                ddtree_k = std::max(1, std::atoi(v));
            }
            if (const char * v = std::getenv("LLAMA_DFLASH_DDTREE_BUDGET")) {
                ddtree_budget = std::max(1, std::atoi(v));
            }
            if (const char * v = std::getenv("LLAMA_DFLASH_DDTREE_TEMP")) {
                ddtree_temp = std::max(1e-3f, (float) std::atof(v));
            }
            if (const char * v = std::getenv("LLAMA_DFLASH_DDTREE_CHAIN_SEED")) {
                ddtree_chain_seed = !(v[0] == '0');
            }
        ...
        for (auto & s : smpls) {
            common_params_sampling sparams;
            sparams.no_perf  = false;
            // When DDTree is on, keep a larger top-K so candidate fallback stays useful.
            sparams.top_k    = ddtree_mode ? std::max(10, ddtree_k) : 10;
            sparams.samplers = { COMMON_SAMPLER_TYPE_TOP_K };
    ...
    const common_ddtree * get_tree(llama_seq_id seq_id) const {
        if (seq_id < 0 || seq_id >= (llama_seq_id) ddtree_last.size()) {
            return nullptr;
        }
    ...
    void begin(llama_seq_id seq_id, const llama_tokens & prompt) override {
        ...
        // Per-request verify host timers (begin_batch / process_ubatch / reuse).
        llama_dflash_clear_verify_timers(params.ctx_tgt);
    ...
    void draft(common_speculative_draft_params_vec & dparams) override {
        ...
        n_draft_blocks++;
        n_draft_block_tok += (size_t) batch.n_tokens;

        // decode all sequence's noise block in a single batch
        // First sample() below forces completion of the draft graph; attribute
        // wall time up to (and including) that first sample to decode, the rest to sample.
        const int64_t t_dec0 = ggml_time_us();
        int ret = llama_decode(ctx_dft, batch);
        if (ret != 0) {
            t_draft_decode_us += ggml_time_us() - t_dec0;
            LOG_WRN("%s: llama_decode returned %d\n", __func__, ret);
            return;
        }

        int64_t t_after_first_sample = -1;
        int64_t t_ddtree_local = 0;
        ...
        t_ddtree_us += t_ddtree_local;

        const int64_t t_end = ggml_time_us();
        if (t_after_first_sample < 0) {
            t_draft_decode_us += t_end - t_dec0;
        } else {
            t_draft_decode_us += t_after_first_sample - t_dec0;
            // sample time excludes pure tree-build (tracked separately) when possible
            const int64_t sample_span = t_end - t_after_first_sample;
            t_draft_sample_us += std::max<int64_t>(0, sample_span - t_ddtree_local);
        }
    }
...
void common_speculative_begin(common_speculative * spec, llama_seq_id seq_id, const llama_tokens & prompt) {
    ...
    for (auto & impl : spec->impls) {
        // Per-generation accumulators so print_stats / get_perf describe this request.
        // (n_parallel > 1 shares one spec; timers are process-global for that case.)
        if (impl->gen_perf) {
            impl->n_call_begin   = 0;
            impl->n_call_draft   = 0;
            impl->n_call_process = 0;
            impl->n_call_accept  = 0;
            impl->n_gen_drafts   = 0;
            impl->n_acc_drafts   = 0;
            impl->n_gen_tokens   = 0;
            impl->n_acc_tokens   = 0;
            ...
            impl->t_process_us = 0;
            impl->t_accept_us  = 0;
            impl->t_draft_decode_us = 0;
            impl->t_draft_sample_us = 0;
            impl->t_ddtree_us       = 0;
            impl->n_draft_blocks    = 0;
            impl->n_draft_block_tok = 0;
            impl->n_draft_out_tok   = 0;
            impl->n_ddtree_builds   = 0;
            impl->n_ddtree_nodes    = 0;
            impl->n_process_combined   = 0;
            impl->n_process_standalone = 0;
        }
    ...
    for (const auto & impl : spec->impls) {
        ...
        if (impl->gen_perf) {
            ...
            oss << std::fixed << std::setprecision(3)
                << impl->t_begin_us / 1000.0 << ", "
                << impl->t_draft_us / 1000.0 << ", "
                << impl->t_process_us / 1000.0 << ", "
                << impl->t_accept_us / 1000.0;
            str_perf = ", dur(b,draft,inject,a) = " + oss.str() + " ms";
        } else {
            str_perf = "";
        }
        ...
        SPC_TRC("statistics %16s: #calls(b,g,p,a) = %4zu %6zu %6zu %6zu, #gen drafts = %6zu, #acc drafts = %5zu, #gen tokens = %6zu, #acc tokens = %5zu%s%s\n",
                common_speculative_type_to_str(impl->type).c_str(),
                impl->n_call_begin, impl->n_call_draft, impl->n_call_process, impl->n_call_accept,
                impl->n_gen_drafts,
                impl->n_acc_drafts,
                impl->n_gen_tokens,
                impl->n_acc_tokens,
        ...
        if (common_speculative_type_is_dflash_family(impl->type) && impl->gen_perf &&
            ...
            const double dec_ms     = impl->t_draft_decode_us / 1000.0;
            const double smp_ms     = impl->t_draft_sample_us / 1000.0;
            const double n_draft    = (double) std::max<size_t>(impl->n_call_draft, 1);
            const double n_proc     = (double) std::max<size_t>(impl->n_call_process, 1);
            const double n_blocks   = (double) std::max<size_t>(impl->n_draft_blocks, 1);
            const double mean_block = impl->n_draft_blocks > 0
                ? (double) impl->n_draft_block_tok / (double) impl->n_draft_blocks : 0.0;
            const double mean_out   = impl->n_draft_blocks > 0
                ? (double) impl->n_draft_out_tok / (double) impl->n_draft_blocks : 0.0;

            const double tree_ms = impl->t_ddtree_us / 1000.0;
            const double n_trees = (double) std::max<size_t>(impl->n_ddtree_builds, 1);
            const double mean_nodes = impl->n_ddtree_builds > 0
                ? (double) impl->n_ddtree_nodes / (double) impl->n_ddtree_builds : 0.0;

            SPC_INF(
                "dflash timing: draft=%7.1f ms (%5.2f ms/call, decode=%5.2f sample=%5.2f ddtree=%5.2f)  "
                "inject=%7.1f ms (%5.2f ms/call, combined=%zu standalone=%zu)  "
                "blocks=%zu mean_block_tok=%.2f mean_draft_out=%.2f\n",
                draft_ms, draft_ms / n_draft, dec_ms / n_blocks, smp_ms / n_blocks, tree_ms / n_trees,
                inject_ms, inject_ms / n_proc,
...
bool common_speculative_get_perf(
    ...
    const common_speculative_impl * impl = nullptr;
    if (type == COMMON_SPECULATIVE_TYPE_NONE) {
        impl = spec->impls.front().get();
    } else {
        for (const auto & candidate : spec->impls) {
    ...
    out.type              = impl->type;
    out.n_call_draft      = impl->n_call_draft;
    out.n_call_process    = impl->n_call_process;
    out.t_draft_us        = impl->t_draft_us;
    out.t_process_us      = impl->t_process_us;
    out.t_draft_decode_us = impl->t_draft_decode_us;
    out.t_draft_sample_us = impl->t_draft_sample_us;
    out.t_ddtree_us       = impl->t_ddtree_us;
    out.n_draft_blocks    = impl->n_draft_blocks;
    out.n_draft_block_tok = impl->n_draft_block_tok;
    out.n_draft_out_tok   = impl->n_draft_out_tok;
    out.n_ddtree_builds   = impl->n_ddtree_builds;
    out.n_ddtree_nodes    = impl->n_ddtree_nodes;
...
bool common_speculative_get_tree(
    ...
    if (spec == nullptr || out_tree == nullptr) {
        return false;
    }
    for (auto & impl : spec->impls) {
        ...
        const common_ddtree * t = dflash->get_tree(seq_id);
        if (t) {
            *out_tree = t;
            return true;
        }
