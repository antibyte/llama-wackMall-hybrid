...
extern "C" {
    ...
    // are zeroed. all weights are [., ., n_expert] expert tensors; gate/up
    ...
    // f32 tensor holding 0.0f (reference it to keep the op in the graph)
    GGML_API struct ggml_tensor * ggml_moe_count(
            struct ggml_context * ctx,
            struct ggml_tensor  * ids,
            struct ggml_tensor  * counts);
