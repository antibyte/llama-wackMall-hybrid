...
static void expert_trace_dump(const struct ggml_tensor * src0, const struct ggml_tensor * ids) {
    if (getenv("LLAMA_EXPERT_TRACE_DEBUG")) {
        fprintf(stderr, "HOOK classic: src0='%s' ids=[%d,%d]\n", src0->name, (int) ids->ne[0], (int) ids->ne[1]);
    }
    ...
    if (f == NULL || ids->type != GGML_TYPE_I32) {
        static bool once = false;
        if (!once && getenv("LLAMA_EXPERT_TRACE")) {
            once = true;
            fprintf(stderr, "expert_trace: classic path hit (f=%p ids_type=%d)\n", (void *) f, ids->type);
        }
        return;
    }
    ...
    if (sscanf(src0->name, "blk.%d.", &il) != 1) {
        ...
        if (!reported) {
            reported = true;
            fprintf(stderr, "expert_trace: skip src0='%s' type=%d\n", src0->name, src0->type);
        }
        return;
    }

    // up/gate/down share the ids tensor: record once per layer
    static const struct ggml_tensor * prev_ids = NULL;
    if (ids == prev_ids) {
        return;
    }
    prev_ids = ids;
...
static void ggml_compute_forward_mul_mat_id_cold(
    ...
    if (ith == 0) {
        // hot slots are not computed below, so the whole dst is zeroed first
        memset(dst->data, 0, ggml_nbytes(dst));

        // initialize matrix_row_counts
        memset(matrix_row_counts, 0, n_as*sizeof(int64_t));

        // group rows by src0 matrix, skipping hot experts
        for (int64_t iid1 = 0; iid1 < ids->ne[1]; ++iid1) {
            for (int id = 0; id < n_ids; ++id) {
                const int32_t i02 = *(const int32_t *) ((const char *) ids->data + iid1*ids->nb[1] + id*ids->nb[0]);
...
// fused cold-expert MoE: computes down(silu(gate(x)) * up(x)) only for slots
// whose expert is cold (cold_mask[e] == 1); hot slots are zeroed.
// src = {gate, up, down, x, ids, cold_mask, counts(optional)}
static void ggml_compute_forward_moe_count(
        const struct ggml_compute_params * params,
              struct ggml_tensor * dst) {
    if (params->ith != 0) {
        return;
    ...
    struct ggml_tensor       * counts = dst->src[1];

    int32_t * cnt = (int32_t *) counts->data;
    const int n_as = counts->ne[0] - 1;
...
static void ggml_compute_forward_moe_cold(
    ...
    ggml_from_float_t const from_fx   = type_traits_cpu[vdt_g].from_float;
    ggml_vec_dot_t    const vec_dot_d = type_traits_cpu[type_d].vec_dot;
    enum ggml_type    const vdt_d     = type_traits_cpu[type_d].vec_dot_type;
    ggml_from_float_t const from_fa   = type_traits_cpu[vdt_d].from_float;
...
static void ggml_compute_forward(struct ggml_compute_params * params, struct ggml_tensor * tensor) {
    ...
    switch (tensor->op) {
        ...
        case GGML_OP_MOE_COUNT:
            {
                ggml_compute_forward_moe_count(params, tensor);
            } break;
