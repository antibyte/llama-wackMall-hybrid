...
QWEN_MODEL="$HOME/models/qwen3.6-35b-a3b-mtp/Qwen3.6-35B-A3B-UD-Q4_K_M.gguf"
QWEN_DRAFT="$HOME/models/qwen3.6-35b-a3b-mtp/Qwen3.6-35B-A3B-DFlash-Q4_K_M.gguf"
QWEN_NAME="qwen3.6-35b-a3b-hybrid"
...
BW_PROFILE="$PROJECT_ROOT/profiles/gtx1660-expert-bw.json"
...
die() {
    printf 'router1660.sh: %s\n' "$*" >&2
    exit 1
}

if [[ $# -ne 0 ]]; then
    die "Keine Kommandozeilenparameter: Einstellungen oben in router1660.sh aendern."
fi

case "$PROFILE_KIND" in
    general)    PROFILE="$PROFILE_GENERAL" ;;
    specialist) PROFILE="$PROFILE_SPECIALIST" ;;
    *)          die "PROFILE_KIND muss general oder specialist sein." ;;
esac
...
RUNTIME="$PROJECT_ROOT/.router-runtime/1660"
...

{
    ...
    printf 'LLAMA_CMOE_PREFILL_UBATCH=1856\n'
...
{
    printf '# Ling-3.0-tiny, start-ling-tiny.sh getenv knobs\n'
...
cat <<EOF
llama-wackMall-hybrid router 1660
  server:   $SERVER
  listen:   $HOST:$PORT
