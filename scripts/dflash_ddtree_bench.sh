...
#   3) DDTree with chain verification
#   4) DDTree with single-batch tree verification at two node budgets
set -Eeuo pipefail
...
# mode configs: name|p_min|ddtree|tree_verify|budget|k|full_chain
MODES=(
  "chain_pmin|0.75|0|0|12|8|0"
  "full_chain|0.0|0|0|12|8|0"
  "ddtree_chain_b12|0.0|1|0|12|8|1"
  "ddtree_tree_b8|0.0|1|1|8|8|1"
  "ddtree_tree_b12|0.0|1|1|12|8|1"
)

printf '%s\n' \
  'mode,n_max,decode_tps,spec_acceptance,mean_accepted_length,draft_tree_nodes,tree_nodes_per_call,draft_ms,inject_ms,verify_ms,draft_decode_ms,draft_sample_ms,n_draft_calls,n_inject_calls,n_verify_calls,draft_ms_per_call,inject_ms_per_call,verify_ms_per_call,step_ms_est,tokens_per_step_est,n_process_combined,n_process_standalone,predicted_n,results_dir' \
  > "$SUMMARY_CSV"

port_i=0
for mode_spec in "${MODES[@]}"; do
  IFS='|' read -r mode_name p_min ddtree tree_verify budget k full_chain <<< "$mode_spec"
  for n_max in $NMAX_LIST; do
    port_i=$((port_i + 1))
    port=$((PORT_BASE + port_i))
    res="$OUT_ROOT/${mode_name}_n${n_max}"
    echo "=== mode=$mode_name n_max=$n_max p_min=$p_min ddtree=$ddtree tree_verify=$tree_verify budget=$budget port=$port ===" | tee -a "$OUT_ROOT/run.log"

    export SPEC_DRAFT_N_MAX="$n_max"
    export SPEC_DRAFT_P_MIN="$p_min"
    export LLAMA_DFLASH_DDTREE="$ddtree"
    export LLAMA_DFLASH_TREE_VERIFY="$tree_verify"
...
mean_len_f = 1.0 + draft_acc / n_v if n_v else 0.0
tree_nodes = int(t.get("draft_tree_nodes") or 0)
tree_nodes_pc = tree_nodes / n_v if n_v else 0.0
...
lines = [
    ...
    "",
    "Compares classic chain, DDTree chain verification, and isolated tree verification.",
    "",
    "| mode | n_max | tok/s | acc | mean_len | tree nodes/call | draft ms/call | verify ms/call | step ms | tokens/step |",
    "|------|------:|------:|----:|---------:|-----------:|--------------:|---------------:|--------:|------------:|",
]
...
for r in rows:
    ...
    lines.append(
        ...
        f"{float(r['mean_accepted_length']):.2f} | {float(r['tree_nodes_per_call']):.2f} | {float(r['draft_ms_per_call']):.2f} | "
        f"{float(r['verify_ms_per_call']):.2f} | {float(r['step_ms_est']):.2f} | "
        f"{float(r['tokens_per_step_est']):.2f} |"
    )
...
if base:
    ...
    cmp.append("")
    cmp.append("| mode | n_max | delta tok/s | delta % | mean_len |")
    cmp.append("|------|------:|--------:|---:|---------:|")
    for r in rows:
...
notes = [
    ...
    "- **ddtree_chain_b12**: exact top-K DDTree construction, then chain verification.",
    "- **ddtree_tree_***: exact DDTree construction and isolated tree-attention verification.",
    "- Tree verification includes the accepted-path commit decode in verify time.",
    "- Compare node budgets on the target GPU because optimal width is hardware-dependent.",
