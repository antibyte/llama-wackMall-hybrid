...
ggml_tensor * llm_graph_context::build_lora_mm_id(
          ...
          ggml_tensor * ids,
          ggml_tensor * w_s,
          ggml_tensor * hot_ids) const {
    ggml_tensor * res = llama_expert_tier::build_mul_mat_id(ctx0, w, cur, ids, hot_ids);
    (void)w_s;
    for (const auto & lora : *loras) {
        llama_adapter_lora_weight * lw = lora.first->get_weight(w);
        if (lw == nullptr) {
...
ggml_tensor * llm_graph_context::build_moe_ffn(
    ...
    if (hparams.n_expert_groups > 1 && n_tokens > 0) {
        ...
        ggml_tensor * group_scores = arch == LLM_ARCH_BAILINGMOE3 ?
                ggml_top_k(ctx0, selection_groups, 2) :
                ggml_argsort_top_k(ctx0, selection_groups, 2); // [2, n_expert_groups, n_tokens]
        ...
        ggml_tensor * expert_groups = arch == LLM_ARCH_BAILINGMOE3 ?
                ggml_top_k(ctx0, group_scores, hparams.n_group_used) :
    ...
    // expert tiering: fused cold path; when active, the build_lora_mm_id calls
    // below compute only the hot slots and the cold op (created after the down
    // matmul so the CPU split overlaps GPU hot work) covers the cold ones
    ...
    ggml_tensor * hot_ids = llama_expert_tier::build_hot_ids(ctx0, gw, selected_experts);
    ...
    if (gate_up_exps) {
        ...
        ggml_tensor * gate_up = build_lora_mm_id(gate_up_exps, cur, selected_experts, up_exps_s, hot_ids); // [n_ff*2, n_expert_used, n_tokens]
        ...
        up = build_lora_mm_id(up_exps, cur, selected_experts, up_exps_s, hot_ids); // [n_ff, n_expert_used, n_tokens]
        cb(up, "ffn_moe_up", il);
        ...
        if (gate_exps) {
            cur = build_lora_mm_id(gate_exps, cur, selected_experts, gate_exps_s, hot_ids); // [n_ff, n_expert_used, n_tokens]
            cb(cur, "ffn_moe_gate", il);
        } else {
            cur = up;
        }
    ...
    experts = build_lora_mm_id(down_exps, cur, selected_experts, down_exps_s, hot_ids); // [n_embd, n_expert_used, n_tokens]
    ...
    if (moe_cold) {
        ...
        ggml_tensor * cnt = llama_expert_tier::build_moe_count(ctx0, down_exps, selected_experts);
        if (cnt) {
            experts = ggml_add(ctx0, experts, cnt); // keep the count op (adds 0.0f)
        }
    }
