    ...
    if (size_done >= size_data) {
        ...
        if (use_mmap) {
            ...
            for (uint32_t idx = 0; idx < mappings.size(); idx++) {
                ...
                const size_t pin_last = mmap_used.second != 0 ? mmap_used.second : mapping->size();
                if (pin_last > mmap_used.first) {
                    const size_t n_registered = mapping->register_host(mmap_used.first, pin_last, reg_fn, unreg_fn);
                    if (n_registered > 0) {
