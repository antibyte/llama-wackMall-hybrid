#!/usr/bin/env bash
#
# llama-wackMall-hybrid GTX 1660 Ti launcher for Ornith-1.5-35B-A3B
# Same settings as start1660.sh (Qwen 3.6 35B-A3B), only model path and alias differ.
#
# Alle Einstellungen werden in diesem Block geaendert. Das Script nimmt bewusst
# keine Kommandozeilenparameter und keine externen Overrides an. Nach einer
# Aenderung einfach erneut ./start-ornith.sh ausfuehren.
#
...
MODEL="$HOME/models/ornith-1.5-35b-a3b/Ornith-1.5-35B-A3B-AD-Q4_K-IQ4_XS.gguf"  # target GGUF
...
MODEL_ALIAS="ornith-1.5-35b-a3b"  # model name advertised by the OpenAI-compatible API
...
# The Pascal MMVQ experiment is intentionally not a runtime variable. It is a
...
GGML_CUDA_REGISTER_HOST="0"  # mmap pin; 21 GiB cudaHostRegister failed on 1660 Ti
GGML_SCHED_PREFETCH_EXPERTS="0"  # n-cpu-moe pp512 +58%; hybrid decode unchanged; 1=3 slots
...
die() {
    printf 'start-ornith.sh: %s\n' "$*" >&2
    exit 1
}

if [[ $# -ne 0 ]]; then
    die "Keine Kommandozeilenparameter: Einstellungen oben in start-ornith.sh aendern."
fi
...
if [[ ( "$DRAFT_TYPE_K" == turbo4_k || "$DRAFT_TYPE_V" == turbo4_k ) && "$LLAMA_TURBO4_DRAFT_EXPERIMENTAL" != 1 ]]; then
    die "Draft-Turbo4 (MTP oder DFlash) benötigt LLAMA_TURBO4_DRAFT_EXPERIMENTAL=1."
fi
