...
# Target und MTP-Draft verwenden beide den abgesicherten Turbo4-KV-Pfad.
...
# Paths
SERVER="$HOME/src/llama-wackMall-turboquant-phase1/build-turbo-fastconvert-sm75/bin/llama-server"  # current Turbo4-enabled sm_75 executable with draft-KV support
MODEL="$HOME/models/qwen3.6-35b-a3b-mtp/Qwen3.6-35B-A3B-UD-Q4_K_M.gguf"  # target GGUF
PROFILE="$PROJECT_ROOT/benchmark-results/profile-corpus-train8-512-20260802T124500Z/general-profile.csv"  # heat/ranking CSV
...
DRAFT_TYPE_K="q4_0"  # 8-bit MTP-draft K cache; valid ggml type name uses digit zero
DRAFT_TYPE_V="q4_0"  # 8-bit MTP-draft V cache; requires Flash Attention
...
LOAD_MODE="none"  # measured loading/runtime winner; "none" was 1.87% slower and reduced S to 32
...
CMOE_BATCH="768"  # largest measured 1660-Ti value that retains all 33 fixed slots
CMOE_UBATCH="376"  # balanced prefill/decode winner with the default VRAM reserve
...
GGML_CUDA_CONCAT_NONCONT_BLOCK_SIZE="0"  # flat CONCAT supersedes the block-size-only experiment
...
llama-wackMall-hybrid start
  ...
  Turbo4:       V-guard=$LLAMA_TURBO4_V_EXPERIMENTAL MTP-guard=$LLAMA_TURBO4_MTP_EXPERIMENTAL draft-guard=$LLAMA_TURBO4_DRAFT_EXPERIMENTAL FP16-threshold=$GGML_CUDA_TURBO4_F16_PREFILL_MIN_BATCH Q8-layers=${LLAMA_TURBO4_Q8_FALLBACK_LAYERS:-none}
