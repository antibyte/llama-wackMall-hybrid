...
#
# Generated by tools/hybrid_autotune at 2026-08-11T13:28:17Z
...
#
#   GPU=NVIDIA GeForce GTX 1080 VRAM=8.00GiB sm=6.1 t=1.00
#   S=58 n_max=8 prefill=1024 batch=128 draft=turbo4_k load=none
#   threads=4 ctx=32768 pascal=True
...
# Paths
SERVER="$PROJECT_ROOT/build-turbo-opt-sm61/bin/llama-server"  # current Turbo4-enabled native sm_75 Release executable
MODEL="/root/atomic-nextn-good/models/Qwen3.6-35B-A3B-UDT-Q4_K_XL_MTP.gguf"  # target GGUF
SPEC_MODE="dflash"  # none | mtp | dflash; DFlash winner: n_max=2 and p_min=0.75
SPEC_DRAFT_MODEL="/root/atomic-nextn-good/models/Qwen3.6-35B-A3B-DFlash-Q4_K_M.gguf"  # DFlash GGUF sidecar
...
PROFILE_GENERAL="$PROJECT_ROOT/1"
...
SPEC_DRAFT_N_MAX="8"  # DFlash-only draft block size
...
REASONING_BUDGET="512"  # measured quality/latency compromise; clients may request another value
...
THREADS="4"  # target generation CPU workers; test physical-core/SMT alternatives
THREADS_BATCH="4"  # CPU workers for prompt/batch processing
DRAFT_THREADS="4"  # CPU workers for the speculative draft context
DRAFT_THREADS_BATCH="4"  # draft workers for draft prompt/batch processing
...
GPU_ARCH="6.1"  # architecture used to choose an auto batch default
...
CMOE_DECODE_BATCH="128"  # optional logical batch used only during token generation
CMOE_DECODE_UBATCH="128"  # optional physical ubatch used only during token generation

# Prompt-cache controls
CTX_CHECKPOINTS="0"  # keep up to four host-side context checkpoints
...
LLAMA_EXPERT_S="58"  # leaves production headroom for CUDA graph instances
...
LLAMA_EXPERT_VRAM_RESERVE_MIB="512"  # runtime auto-fit reserve; forced S remains capped if necessary
...
GGML_CUDA_MOE_MULTI_FUSION="0"  # fuse Gate+Up+GLU for 2-4 target tokens on sm_75+
GGML_CUDA_MOE_COMBINE_FUSION="0"  # exact combine fusion measured neutral (+0.07%)
GGML_CUDA_MMVQ_Q8_NCOLS3_ROWS="0"  # 3x2K winner: 46.831 -> 47.436 token/s
...
GGML_CUDA_MMVQ_Q6_K_NCOLS1_ROWS="0"  # Q6_K output projection, one-token graph
...
GGML_CUDA_MMVQ_Q6_K_NCOLS3_ROWS="0"  # Q6_K output projection, MTP-2 verify graph
...
GGML_CUDA_CONCAT_NONCONT_BLOCK_SIZE="0"  # active together with flat dim-0 in the confirmed 3x2K MTP-2 winner
GGML_CUDA_CONCAT_NONCONT_FLAT_DIM0="0"  # 3x2K winner: 47.794 -> 48.599 token/s
