...
- `child-env-file` (path): `KEY=VALUE` file merged into the child `llama-server` environment (`-u KEY` or `UNSET KEY` removes a variable). Use this for getenv-only knobs such as `LLAMA_EXPERT_S` that are not CLI arguments.
